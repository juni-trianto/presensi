    ...
    <div id="app">
        ...
        <div class="sidebar">
            <section class="cameras">
                    <h2>Scan Qr Code Untuk Absen Masuk</h2>
                <ul>
            ...
            <section class="scans">
                ...
                <transition-group name="scans" tag="ul">
                    <form method="post" name="frm_ajax" v-for="scan in scans" :key="scan.date" :title="scan.content">
                        <textarea style="display:none;" name="hasil" type="hidden" id="hasil" cols="30" rows="10" v-if="scan.content">{{ scan.content }}</textarea>
        ...
        function kirim_form() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    console.log(data);
                    if (data != 1) {
                        window.location = "<?= base_url('user/qr_code_masuk') ?>";
                    } else {
                        <?php $this->session->set_userdata(['status_absen_masuk' => 'Oke']); ?>
                        window.location = "<?= base_url('user/absen_masuk') ?>";
